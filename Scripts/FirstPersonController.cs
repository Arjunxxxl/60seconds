...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void Start()
        {
			...
			gunflash.GetComponent<ParticleSystem>().Stop();
		...
		{	
			...
			if (Input.GetButton("Fire1") && Time.time > nextFire)
			{
				...
				gunfire.Play ();
				clones = Instantiate(shots, shotspawn.position, shotspawn.rotation) as GameObject;
			}
        }
		...
		void OnTriggerEnter(Collider col){
			...
			if (col.gameObject.tag == "coin") {
				if (current == "dastv0.1") {
					StartCoroutine (level ("level2")); 
				}else if (current == "level2") {
					StartCoroutine (level ("level3"));
				}else if (current == "level3") {
					StartCoroutine (level ("level4"));
				}else if (current == "level4") {
					StartCoroutine (level ("level5"));
